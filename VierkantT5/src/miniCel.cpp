...
miniCel::miniCel(int vorm_, float speed_, float Ibh_x, float Ibh_y, float V_loca_x, float V_loca_y, float Iloca_x, float Iloca_y)
{
	...
	oorsprongIlocation.set(V_loca_x + Iloca_x, V_loca_y + Iloca_y);
}

void miniCel::resetLocation()
{
	// Zet alles terug naar de eerste locatie die hij kreeg toen het programma begon.
	Ilocation = oorsprongIlocation;
	Mzijde = oorspongMzijde; 
	state = oorsprongState;
}

void miniCel::display(int kleur_)
{
	//int kleur = ofMap(kleur_, 0, 1023, 0, 255);
	ofSetColor(75, kleur_-150, kleur_);
...
void miniCel::move(float loca_x, float loca_y, float bh_x, float bh_y,int state_, int Mzijde_, float V_speed, float snelheidpotmeter_)
{
	...
	speed = ofMap(snelheidpotmeter_, 0, 1023, -4, 6); 

	if (geit == false)
	{
		oorsprongState = state_;
		oorspongMzijde = Mzijde_;
		state = state_; 
		geit = true;
	}
