...
VierkantCel::VierkantCel(int V_vorm_, float V_speed_, float V_bh_x, float V_bh_y, float Mloca_x, float Mloca_y, float Vloca_x, float Vloca_y)
{
	...
	oorsprongIlocation.set(Mloca_x + Vloca_x, Mloca_y + Vloca_y);
}
...
void VierkantCel::minicelDisplay(int pot1_)
{
	for (int i = 0; i < 4; i++)
	{
		mCel[i].display(pot1_); // Geeft locatie van vierkant door aan miniCel	
	}
}
...
void VierkantCel::ResetLocatie()
{
	mCel[0].resetLocation();
	mCel[1].resetLocation();
	mCel[2].resetLocation();
	mCel[3].resetLocation();

	V_location = oorsprongIlocation;
	V_state = oorsprongV_state;
}

void VierkantCel::Vdisplay(int alfa_)
{
	ofSetColor(80, 4, 131,alfa_);
	ofRect(V_location.x, V_location.y, V_bh.x, V_bh.y);
}

void VierkantCel::move(float Mloca_x, float Mloca_y, float Mbh_x, float Mbh_y, int Vstate_, float M_speedx, float M_speedy , float potmeter_snelheid)
{
	V_speed = ofMap(potmeter_snelheid, 0, 1023, 0, 6);

	if (V_geit == false)
	{
		oorsprongV_state = Vstate_;
		V_state = Vstate_;
		V_geit = true;
	}
